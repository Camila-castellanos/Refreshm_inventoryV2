  ...
  try {
    ...
    if (res && res.data && Array.isArray(res.data.items)) {
      ...
      tableData.value = tableData.value.map((row) => {
        const match = updated.find((u: any) => {
          // try to match by unique id if present, otherwise fallback to first matching imei or index
          if (row.id && u.id) return row.id === u.id;
          if (row.imei && u.imei) return row.imei === u.imei;
          return false;
        });
        if (match && match.selling_price != null) {
          row.selling_price = match.selling_price;
        }
        return row;
      });
      toast.add({ severity: 'success', summary: 'Success', detail: 'Selling prices generated by server.', life: 3000 });
    } else {
