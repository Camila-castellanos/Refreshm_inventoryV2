<template>
  ...
  >
  <span v-if="props.colIndex === 0" class="text-center w-full">{{ props.rowIndex + 1 }}</span>
  <DatePicker v-else-if="props.column.columnType === 'date'" showIcon v-model="model[props.prop]" class="!w-full" dateFormat="yy-mm-dd" :max-date="new Date()" />
  <Select v-else-if="props.column.columnType === 'select'" v-model="model[props.prop]" :options="props.column.source" optionLabel="label" optionValue="value" class="w-full">
    <template #footer>
